...
sum_tree(leaf(V), Sum):-
    Sum is V.
sum_tree(tree(Left, Val, Right), Sum):-
    sum_tree(Left, S1), 
    sum_tree(Right, S2), 
    Sum is Val + S1 + S2.
...
naive_flatten_tree(leaf(Node), Flattened):-
    append([], [Node], Flattened).
naive_flatten_tree(tree(Left, Val, Right), Flattened):-
    naive_flatten_tree(Left, P),
    naive_flatten_tree(Right, S),
    append(P, [Val|S], Flattened).
...
flatten_tree(nil , []).
flatten_tree(leaf(Node), Flattened):-
    append([], [Node], Flattened).
flatten_tree(tree(Left, Val, Right), Flattened):-
    flatten_tree(Left, P),
    flatten_tree(Right, S),
    append(P, [Val|S], Flattened).
